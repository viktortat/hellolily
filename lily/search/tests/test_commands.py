...
from lily.search.registries import DocumentRegistry
from lily.management.commands.search_index import Command
...
class SearchIndexTestCase(TestCase):
    ...
    def test_get_models(self):
        cmd = Command()
        with patch('lily.management.commands.search_index.registry', self.registry):
            self.assertEqual(
                cmd._get_models(['foo']),
    ...
    def test_delete_foo_index(self):
        with patch('lily.management.commands.search_index.registry', self.registry):
            with patch('lily.management.commands.search_index.input', Mock(return_value="y")):
